	...
	if (empty($id_user)) { 
		...
		$num_rows = mysqli_num_rows(mysqli_query($con, "SELECT * FROM profil WHERE id_user='".$id_user."'"));
	       ...
	       if ($num_rows == 0){
				...
				$query2 = mysqli_query($con, "UPDATE profil SET foto='$tes',nama_ibu='$nama_ibu',email='$email',nama_anak='$nama_anak',alamat='$alamat',tgl_lahir='$tgl_lahir',jenkel='$jenkel' WHERE id_user='$id_user'");
